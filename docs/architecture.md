...
自 v3.6 起，capabilities 改由 **`manifest.json`** 定義，不再從 TD Custom Parameters 讀取。`GET /` 只需回傳 `module` 名稱，Node.js 從對應的 manifest 取得能力設定。

TD 的 `SupportsRecording` / `SupportsSnapshot` Custom Parameters 可選擇保留作為 TD 內部參考，但不再影響 Node.js 的判斷邏輯。

### 輸出

- **NDI Out**：即時串流給 NDI 6 Webcam 虛擬驅動，瀏覽器透過 `getUserMedia` 讀取預覽
- **File Saver TOP**：存檔至 `sessions/{sessionID}/raw_{index}.png`，路徑由 `/reset` 端點傳入的 `sessionID` 決定
...
1. 依 `layout` 物件讀取畫布尺寸、照片槽位、widgets
